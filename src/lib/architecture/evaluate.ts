...
export function evaluateMenuBuilder(input) {
  ...
  const zeroProofDirection =
    input.beverageRoute === 'alcoholic'
      ? 'Zero-proof is optional; if offered it must still read as intentional, not a consolation.'
      : 'Zero-proof is equal-status and must be locked into the Equal role before handoff.';
  ...
  // ---- Drink track (Architecture beverage route) -------------------------
  const allDrinkIds = new Set(Object.values(DRINK_ROLE_CANDIDATES).flat());
  const drinkRoles = ['arrival', 'volume', 'cut', 'equal', 'station'];
  const drinkPlan = [];
  const selectedDrinkIds = [];
  const roleNameMap = {};

  for (const role of drinkRoles) {
    const pick = pickForRole(
      role,
      allDrinkIds,
      input.beverageRoute || 'both',
      input.lockedEqualId || null
    );
    const primaryId = pick.primaryId;
    if (primaryId) selectedDrinkIds.push(primaryId);
    for (const alt of pick.alternativeIds) {
      if (!selectedDrinkIds.includes(alt)) selectedDrinkIds.push(alt);
    }
    const primaryName = primaryId
      ? primaryId
          .replace(/^cw-drink-|^drink-|^cn-|^pairing-|^mx-|^jp-|^ae-|^fa-|^zh-|^wa-|^sa-|^in-|^lv-|^cb-|^it-|^no-/i, '')
          .replace(/-/g, ' ')
      : null;
    drinkPlan.push({
      role,
      primary: primaryId
        ? {
            id: primaryId,
            name: primaryName,
            blurb: DRINK_ROLE_GUIDANCE[role] || '',
            why: role === 'equal' ? 'Equal-status peer; not a fallback.' : undefined,
            makeAhead: true,
            heat: role === 'station' ? 'varies' : 'none',
            flavorFamilies: [],
            score: 75,
            fitReasons: [DRINK_ROLE_GUIDANCE[role] || role],
          }
        : null,
      alternatives: pick.alternativeIds.slice(0, 3).map((id) => ({
        id,
        name: id
          .replace(/^cw-drink-|^drink-|^cn-|^pairing-|^mx-|^jp-|^ae-|^fa-|^zh-|^wa-|^sa-|^in-|^lv-|^cb-|^it-|^no-/i, '')
          .replace(/-/g, ' '),
    ...
    if (primaryId) roleNameMap[role] = primaryName;
  }

  const lockedEqualId =
    input.lockedEqualId ||
    drinkPlan.find((b) => b.role === 'equal')?.primary?.id ||
    null;

  const hasEqual = Boolean(lockedEqualId) || input.beverageRoute === 'alcoholic';
  const iceHeavy = selectedDrinkIds.some((id) =>
    /margarita|frozen|spritz|punch|cooler|lemonade|agua|cider/i.test(id)
  );
  const hotStation = selectedDrinkIds.some((id) =>
    /mulled|hot-cider|hot-punch|glogg|chai|urn/i.test(id)
  );

  const beverageStress = buildBeverageStress({
    mode: input.beverageRoute || 'both',
    hasEqual,
    iceHeavy,
    hotStation,
    attentionBand: String(input.attentionBand || 'moderate'),
  });

  const beverageRouteResult = {
    roles: roleNameMap,
    lockedEqualId,
    selectedDrinkIds: [...new Set(selectedDrinkIds)],
    stress: beverageStress,
    direction: beverageDirection,
    zeroProofDirection,
    mode: input.beverageRoute || 'both',
  };
  ...
  return {
    ...
    nextActions: [
      ...
      'Lock the Equal drink (or declare alcoholic-only) before handoff.',
    ...
    drinkPlan,
    beverageStressTest: beverageStress,
    selectedDrinkIds: beverageRouteResult.selectedDrinkIds,
    lockedEqualId,
    beverageRouteResult,
  };
}
