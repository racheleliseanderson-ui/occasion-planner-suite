...
export const handoffPacketSchema = z.object({
  ...
  /** Explicit locked beverage route (Architecture drink track → Plan). Optional for backward compatibility. */
  beverageRoute: z
    .object({
      roles: z.record(z.string(), z.string()),
      lockedEqualId: z.string().nullable(),
      selectedDrinkIds: z.array(z.string()),
      stress: z
        .object({
          score: z.number(),
          band: z.string(),
          dimensions: z.record(z.string(), z.number()),
          weakDimensions: z.array(
            z.object({
              dimension: z.string(),
              score: z.number(),
              band: z.string(),
...
export function buildMenuOccasionHandoff(
  ...
  const handoff: HandoffPacket = {
    ...
    zeroProofDirection,
    beverageMode,
    beverageRoute: output.beverageRouteResult
      ? {
          roles: { ...(output.beverageRouteResult.roles || {}) },
          lockedEqualId: output.beverageRouteResult.lockedEqualId ?? output.lockedEqualId ?? null,
          selectedDrinkIds: [...(output.beverageRouteResult.selectedDrinkIds || output.selectedDrinkIds || [])],
          stress: output.beverageRouteResult.stress
            ? {
                score: output.beverageRouteResult.stress.score,
                band: String(output.beverageRouteResult.stress.band),
                dimensions: { ...output.beverageRouteResult.stress.dimensions },
                weakDimensions: [...(output.beverageRouteResult.stress.weakDimensions || [])],
                verdict: output.beverageRouteResult.stress.verdict,
              }
            : undefined,
          direction: output.beverageRouteResult.direction || String(output.beverageDirection || ""),
          zeroProofDirection: output.beverageRouteResult.zeroProofDirection || zeroProofDirection,
          mode: String(output.beverageRouteResult.mode || beverageMode),
        }
      : undefined,
